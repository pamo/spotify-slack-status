...
app.get('/', (req, res) => {
  ...
  response.spotifyMessage = hasSpotifyToken
    ? 'An access token was found.\n'
    : '<a href="/spotify/authorize" class="f4 fw6 db black link hover-blue">Authorize</a>';

  response.slackMessage = slackAccessToken
    ...
    .then(data => {
      ...
      console.log(
        'Retrieved token. It expires in ' +
          Math.floor(spotifyTokenExpirationEpoch - new Date().getTime() / 1000) +
          ' seconds!'
      );
    })
    .then(() => {
...
app.get('/update', (req, res) => {
  ...
  if (!spotifyApi.getAccessToken() || !slackAccessToken) {
    ...
    response.error =
      '<h2>Tokens are not available</h2><a href="/" class="f4 fw6 db black link hover-blue">Reauthenticate</a>';
      ...
      .then(data => {
        if (data.body.is_playing) {
          ...
          return slack.users.profile.set({
            profile: { status_text: logText, status_emoji: ':headphones:' },
          });
        } else {
...
const POLLING_INTERVAL_MILLISECONDS = 30000;
const pollFetch = (path = '/') =>
  setInterval(() => {
